...
/* ── 12 background + logo color pairs ── */
interface ColorPair {
  bg: string;
  logo: string;
}
...
const COLOR_PAIRS: ColorPair[] = [
  { bg: '#FFFFFF', logo: '#171717' },  // White → near-black
  { bg: '#171717', logo: '#FFFFFF' },  // Near-black → white
  { bg: '#EF4444', logo: '#FFFFFF' },  // Red 50 → white
  { bg: '#F97316', logo: '#FFFFFF' },  // Orange 60 → white
  { bg: '#E5E5E5', logo: '#450A0A' },  // Gray 20 → Red 100
  { bg: '#450A0A', logo: '#FB923C' },  // Red 100 → Orange 50
  { bg: '#F5F5F5', logo: '#B91C1C' },  // Gray 10 → Red 70
  { bg: '#DC2626', logo: '#171717' },  // Red 60 → near-black
  { bg: '#7C2D12', logo: '#FAFAFA' },  // Orange 100 → near-white
  { bg: '#404040', logo: '#FB923C' },  // Gray 60 → Orange 50
  { bg: '#EA580C', logo: '#171717' },  // Orange 70 → near-black
  { bg: '#262626', logo: '#EF4444' },  // Gray 80 → Red 50
];
...
export function LogoComposition() {
  ...
  const scheduleNext = useCallback(() => {
    const delay = 1800 + Math.random() * 1200; // 1.8–3s
  ...
  return (
    ...
    >
      ...
      {/* Noise overlay */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.04] mix-blend-overlay [background-image:url('https://grainy-gradients.vercel.app/noise.svg')]" />
    </motion.div>
  );
}
